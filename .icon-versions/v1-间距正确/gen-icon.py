...
and are darkened by blurred fillers. Front rhythm (center column), per the
user's 9-layer model:
...
BAND_W = 15.0                       # shadow band width (~h/2)
...
def build():
    ...
    f.append('<defs>'
             + blur_filter("bBand", 3) + blur_filter("bNotch", 9)
             ...
             + f'<linearGradient id="gBand" x1="0" y1="0" x2="0" y2="1">'
               f'<stop offset="0" stop-color="{BAND_SHADOW}" stop-opacity="0.8"/>'
    ...
    gs = rp(CX, CY[2] + HPS[2] + THICK + 8, Wp * 0.98, HPS[2] * 0.98, CUT)
    f.append(f'<path d="{gs}" fill="{SHADOW_GROUND}" opacity="0.5" filter="url(#bGround)"/>')
    gs2 = rp(CX, CY[2] + HPS[2] + THICK + 26, Wp * 0.92, HPS[2] * 0.92, CUT)
    f.append(f'<path d="{gs2}" fill="{SHADOW_GROUND}" opacity="0.2" filter="url(#bGround2)"/>')
    ...
    # painted back-to-front; the top slab is drawn last (closest to camera) and
    # hides each shadow ring's top even-odd lens. Per gap: dark notch fillers
    # at the wings + a soft shadow filling the h-wide exposed strip of the
    # lower face, darkest at the strip's middle (h/2 below the underside).
    bottom, middle, top = 2, 1, 0
    draw_plate(bottom)
    for i, (up, low) in enumerate(((middle, bottom), (top, middle))):
        notch_cy = (CY[up] + THICK + CY[low]) / 2
        for sx in (-1, 1):
            nf = rp(CX + sx * (Wp - 30), notch_cy, 48, 24, 12)
            f.append(f'<path d="{nf}" fill="{SHADOW_NOTCH}" opacity="0.5" '
                     f'filter="url(#bNotch)"/>')
        exposed = CY[low] + HPS[low] - (CY[up] + HPS[up] + THICK)
        ring = (rp(CX, CY[up] + THICK + exposed, Wp, HPS[up], CUT) + " "
                + rp(CX, CY[up] + THICK, Wp, HPS[up], CUT))
        f.append(f'<clipPath id="cBand{i}"><path d="{plate(CX, CY[low], HPS[low])}"/></clipPath>')
        y_top = CY[up] + HPS[up] + THICK
        y_end = CY[low] + HPS[low]
        gid = f"gBand{i}"
        f.append(f'<linearGradient id="{gid}" gradientUnits="userSpaceOnUse" '
                 f'x1="0" y1="{y_top:.0f}" x2="0" y2="{y_end:.0f}">'
                 f'<stop offset="0" stop-color="{BAND_SHADOW}" stop-opacity="0.2"/>'
                 f'<stop offset="0.5" stop-color="{BAND_SHADOW}" stop-opacity="0.7"/>'
